...
return [
    ...
    */
    ...
    'temporary_file_upload' => [
        ...
        // Livewire's own package default here is ['required', 'file',
        // 'max:12288'] (12MB) — a real, confirmed gap this project hit
        // in practice: it rejects a file BEFORE it ever reaches
        // Filament's own per-field validation (this temp-upload step
        // runs first, on file select/drop, not on form submit), making
        // ProductResource's own video field (max 100MB, per
        // media-domain-design.md §6 / config('services.media.
        // max_video_size_kb')) unusable past 12MB regardless of that
        // config. This is a single GLOBAL rule for every Livewire file
        // upload in the app (no native per-field override), so it's
        // set to the larger of the two real media limits.
        //
        // env() directly here, NOT config('services.media...') —
        // config files load in an unspecified/alphabetical order
        // ('livewire.php' before 'services.php'), so calling config()
        // on another config file from inside this one risks reading it
        // before it's been loaded. Reading the SAME underlying env vars
        // config/services.php itself reads (MEDIA_MAX_IMAGE_SIZE_KB /
        // MEDIA_MAX_VIDEO_SIZE_KB) sidesteps that ordering risk
        // entirely while staying config-driven, not a second hardcoded
        // number that could drift out of sync.
        'rules' => ['required', 'file', 'max:'.max(
            (int) env('MEDIA_MAX_IMAGE_SIZE_KB', 10240),
            (int) env('MEDIA_MAX_VIDEO_SIZE_KB', 102400)
        )],
