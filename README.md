...
| [`packages/EasyCo/Promotions`](packages/EasyCo/Promotions) | Promotion/PromotionScope — customer-entered discount codes (percentage or fixed-amount), with extensible eligibility scoping (brand/category/tag/attribute/product/account, include or exclude) and usage-limit/validity-window fields. Promotion itself has HTTP create/list (`POST`/`GET /promotions`); PromotionScope attach/list/detach over HTTP and Cart integration are both still missing, so a customer cannot yet apply a code to a cart. | [promotions-domain-design.md](packages/EasyCo/documents/promotions-domain-design.md) |
