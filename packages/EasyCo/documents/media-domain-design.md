...
**Status:** v1.0 — design only; nothing in this document is implemented yet. Same framing as `performance-and-channel-strategy.md`: it exists to record decisions and constraints *before* the corresponding code is written, so they aren't rediscovered (or re-litigated) later.
...
Three tables already exist — `catalog_media`, `catalog_product_media`, `catalog_variation_media` (`packages/EasyCo/Catalog/database/migrations/2026_08_23_000012_create_catalog_media_tables.php`), created during the original Catalog design pass. Confirmed by inspection before writing this document: `catalog_media` currently has only `id`, `type` (`image | video | social_image | social_video`), `url`, `alt_text`, timestamps — no disk column, no processing-status column, no generated-variant storage. The two pivots (`catalog_product_media`, `catalog_variation_media`) are a straightforward `product_id`/`variation_id` + `media_id` + `sort_order`, each with a `unique(parent_id, media_id)` constraint. There is no Eloquent model, no domain class, no processing pipeline, and no HTTP surface for any of this today (confirmed by search — no `Media` class exists anywhere in `packages/EasyCo/`).
...
**New package, new migrations directory — an explicit action item, not automatic.** Every other domain (`Pricing`, `Catalog`, `OperationalSales`) owns its migrations under its own package's `database/migrations/`. For `EasyCo\Media` to follow the same pattern, the existing `2026_08_23_000012_create_catalog_media_tables.php` migration (currently physically under `packages/EasyCo/Catalog/database/migrations/`) needs to be relocated to a new `packages/EasyCo/Media/database/migrations/` directory when implementation actually starts — flagged here explicitly so it isn't done inconsistently (e.g. left in Catalog's directory while the domain class lives in a `Media` package, which would be a confusing split of ownership).

**Columns to add to `catalog_media`** (none of these exist today — confirmed by reading the migration in §1): `disk` (string), `processing_status` (string, default `pending`), and a `variants` column to hold the generated WebP renditions from §3/§8. Proposed as a single JSON column rather than a dedicated child table for v1 — the same "smallest model that satisfies the actual need" reasoning `catalog-domain-design.md` §3.3 already applied to attribute definitions: nothing today needs to query "find all media with a `medium` variant of size X," so a child table would add relational overhead with no current payoff. Revisit as a dedicated table only if that kind of per-variant query actually becomes necessary.
