...
**Status:** v1.3 — domain layer, persistence, image-processing pipeline, storage adapter, and HTTP surface are all now implemented. §11 (added in this revision) retroactively documents `ProductMedia.autoplay`, which shipped in an earlier admin-panel task without a design-doc entry at the time, and adds the new one-video-per-product `VideoCountGuard` alongside it. `MediaAsset` (`create()`/`reconstituteFromStorage()`, guarded PENDING→PROCESSING→READY|FAILED transitions, video types permanently READY per §4), `MediaVariant`, `ProductMedia`/`VariationMedia` (the two existing pivots), their Eloquent repositories, `LaravelMediaImageProcessor` + `ProcessMediaAssetJob` (§3), and `LaravelMediaStorageAdapter` (§5) are implemented and tested. HTTP surface: `POST /api/media` (standalone upload); `POST`/`GET`/`PUT .../order`/`DELETE` on both `/api/products/{productId}/media` and `/api/variations/{variationId}/media` (attach, list, reorder, detach — §8.5 records the reorder/detach decisions). **Still genuinely open, checked against the current codebase rather than assumed from memory:** video processing/transcoding (§4, deliberately deferred, not a gap); the `HeroSlide` entity (§2.2/§8) — no migration or model exists yet; the Hero Slider's feature-wide enabled toggle, which waits on the Site settings/logo-favicon model (§2.3/§10) — that model also doesn't exist yet, though a *separate*, generic key-value Site Settings mechanism (`app/Settings/`) has since been built for unrelated purposes and is not wired to either of these; CDN-specific configuration beyond the generic disk abstraction (§10); image moderation (§10); and §7's AI-generated-media disclosure flag, still an open cross-reference, not resolved. Originally framed as "design only... before the corresponding code is written" (see `performance-and-channel-strategy.md`'s same framing) — that framing applied to v1.0; this document now trails real implementation rather than preceding it.
...
### 8.5 Attach/list/detach/reorder HTTP decisions

Three decisions made while adding the list/detach/reorder HTTP surface on top of the existing attach endpoints, recorded here so they aren't re-litigated:

**Detach removes the pivot row only — never the underlying `MediaAsset`.** `ProductMediaController::destroy()`/`VariationMediaController::destroy()` only ever call the pivot repository's `remove()`; `catalog_media` is never touched. This was an open question put to the domain owner, who had no strong preference — the simpler, narrower-scope option was taken deliberately: a `MediaAsset` may legitimately be reused/attached elsewhere (e.g. the same photo on two variations), so a detach cannot assume it's safe to delete the underlying asset. Deleting a `MediaAsset` outright (and deciding what happens to the now-orphaned physical file, §5) is a separate, not-yet-designed concern that would need its own explicit endpoint.

**Reorder takes pivot ids, not media ids, and requires the entire current set as one ordered array — a full replace, not a per-item position patch.** The client sends `{"order": [pivotId, pivotId, ...]}` to `PUT .../media/order`; array index becomes the new `sort_order`, applied to every row inside a single `DB::transaction()`. Precedent: WooCommerce's REST API reorders a product gallery via a full `gallery_image_ids` array, where array order *is* display order. The alternative — a numeric `position` field patched per item — was deliberately rejected: Magento's REST API has a known bug class where per-item `position` updates end up duplicated or out of sync with the real drag-and-drop order, because the updates aren't atomic against each other. A full-array replace inside one transaction avoids that failure mode structurally, not by convention. The provided array must be exactly the current set of pivot ids (compared as sorted arrays) — missing, extra, or foreign-product/variation ids all reject with a 422 before the transaction ever opens; an empty array against a product/variation with no attached media is a legitimate no-op, not an error.

**Detach requires an ownership check — a pivot id alone is not sufficient authorization.** Both `destroy()` methods look up the pivot via `findByProductId($productId)`/`findByVariationId($variationId)` first, not a direct `findById()`-then-delete — a pivot id that exists but belongs to a *different* product/variation is treated identically to a nonexistent one (404), never deleted. Without this check, knowing any pivot id (e.g. sequential integers) would let a caller detach media from an unrelated product/variation just by guessing the URL's parent id.
...
## 11. Retroactive record: `ProductMedia.autoplay`, plus the new one-video-per-product guard

**Two different provenances, stated plainly so neither is misread as the other's:** `ProductMedia.autoplay` (first bullet below) **already shipped** in an earlier admin-panel task (the product-edit-page media-sidebar redesign: separate Main Photo/Gallery Photos/Video fields) without a design-doc entry at the time — this is a retroactive record of existing, running code, not a new decision. `VideoCountGuard` (second bullet) is a **new addition landing in this same revision** — a real gap an architect review surfaced (the single-video-per-product behavior existed only as a UI convention, a single-file field, with no domain-level invariant behind it) — designed and implemented together with this documentation, not documented after the fact.

**`ProductMedia.autoplay` (boolean, `NOT NULL DEFAULT false`)** — a new column on `catalog_product_media`, confirmed via `SHOW CREATE TABLE` against the real dev database: `` `autoplay` tinyint(1) NOT NULL DEFAULT '0' ``. Whether this product's attachment of a video should autoplay on the (not-yet-built) storefront. Lives on `ProductMedia` itself — the per-attachment pivot — not on `MediaAsset`: the same `MediaAsset` could in principle be attached to more than one product, each wanting its own autoplay preference. **Video-specific; meaningless for a photo attachment.** Nothing rejects setting it on a photo pivot anyway — there is no real invariant to protect by forbidding it, mirroring `ProductMedia`'s own existing "not this class's job to add validation the constructor doesn't already have" posture. Set via the admin panel's video upload field (a `Toggle` alongside the single-video `FileUpload`), read/written through `ProductMedia::autoplay()`/`updateAutoplay()` and `EloquentProductMediaRepository`'s existing `save()`/`toDomainProductMedia()` mapping — no new repository method was needed for this column itself (only for the count guard below).

**One video per product — a new, separate `VideoCountGuard`, not a branch inside `ProductMediaCountGuard`.** §6's combined photo/video slot count (`ProductMediaCountGuard`, `max_photos_per_product`, default 10) is **unchanged** and still governs images exactly as before — `VideoCountGuard` is an independent invariant, counting only `MediaType::VIDEO`-typed `ProductMedia` rows for a product via a new `ProductMediaRepository::countByProductIdAndType()` method (a subquery against `catalog_media.type`, no new Eloquent relation introduced for it). Mirrors `ProductMediaCountGuard`/`VariationMediaCountGuard`'s own shape (`assertCanAttach()`, `MediaLimitExceededException`, one small single-purpose guard class per invariant — the same precedent those two already set, rather than parameterizing one guard by type). **Fixed at exactly 1, not merchant-configurable** — unlike its two siblings, `VideoCountGuard`'s constructor takes no injected max and `MediaServiceProvider` reads no `config()` for it, since "at most one video" is a domain rule, not a setting. Wired into `CreateProduct`/`EditProduct`'s write-interception alongside (not replacing) the existing `ProductMediaCountGuard` call, at the same point video attachment happens. `MediaLimitExceededException::forProductVideo(string $productId)` is the new exception factory (no `currentCount`/`maxCount` params, since there is nothing variable to report).

**One real correctness fix this required in `EditProduct::syncMedia()`:** orphaned pivots (a path no longer in the submission — i.e. the user replaced the video) are now detached **before** any new attach is attempted, not after. Without this ordering, replacing the single video would transiently look like attaching a *second* video — the old pivot still present in the database at the moment the guard checks — and be wrongly rejected. This ordering also applies to the shared photo call, though the combined guard's own headroom (default 10) meant the bug was never actually observable there.
