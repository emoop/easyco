...
## 8. Per-line discount allocation — added for returns support, designed not built

`operational-sales-domain-design.md` §3.13 needs each `SaleLine` to record its own share of an order's promotion discount (so a return knows how much of *that line's* money to refund) — this package does not own that allocation math, and does not gain any new entity or field for it. `App\Services\PromotionDiscountCalculator` (app-layer, not this package) already computes the one order-level discount total; §3.13's design extends that SAME method to also return a per-line breakdown. This package's own entities (`Promotion`, `PromotionScope`) supply the inputs that calculation reads (`discount_type`, `discount_value`, `usage_limit_items`) — the same, unchanged role they already play for the order-level total — and stay unaware that a per-line breakdown is even computed from them, consistent with this document's own `App\Services\PromotionDiscountCalculator`/`PromotionValidator` composition boundary (§1, §4) rather than this domain reaching into cross-domain allocation concerns itself.

**REVISED — the actual split-a-total-into-weighted-shares math lives one level lower than `PromotionDiscountCalculator` itself.** §3.13's own review found the calculator's first proposed per-line rule (round each share, let the last line absorb the remainder) produces a **negative share** on a real input (10% over eligible amounts 5, 5, 1 minor units — see §3.13's Promotion allocation rule for the full counter-example and the corrected largest-remainder method). The fix — `Money::allocate(array $weights): array`, recommended on `EasyCo\Pricing\Money` itself — is not a Promotions concept either; it is a pure, stateless Money operation with no dependency on `Promotion`/`PromotionScope`, `discount_type`, or anything else this package owns. `PromotionDiscountCalculator` becomes one of `Money::allocate()`'s callers, not its home — a future POS bill-level discretionary discount (a different domain concern entirely) needs the identical operation and reuses the same method, never a second implementation. This package's own boundary is unchanged by this correction: it still supplies inputs, never allocation math, regardless of which layer that math ends up living in.

**IMPLEMENTED (operational-sales-domain-design.md §3.13's own implementation stage 3):** `Money::allocate()` and `PromotionDiscountCalculator`'s per-line breakdown both exist now. `PromotionDiscountResult::perLineShares()` (positional, never keyed by `variationId`) is real, tested, and always sums to exactly `amount()` — but has **no consumer yet**. `CheckoutOrchestrator` and `CartController` still read only `amount()`/`discountCapped()`/`nominalAmount()`, unchanged. `perLineShares()` is first consumed once `SaleLine`'s `promotionDiscountShare` snapshot field is actually populated at checkout — operational-sales-domain-design.md §3.13's own implementation stage 7 (Checkout writes the full snapshot).
