...
**Found in review, not part of the original design — why the `Payment` row moved into Phase 1 (step 10):** the original design created the `Payment` row in Phase 2, after the adapter answered. A crash, an adapter exception, or any other uncaught failure between Phase 1's commit and that Phase 2 write left an `Order` with **no `Payment` row at all** — not `FAILED`, not `PENDING`, nothing. Worse, a retry of the same cart hit §6's idempotent-replay fast path (the cart was already claimed) and never re-ran Phase 2 at all — an order the customer could never pay for, with no trace of the problem. Writing the row as `PENDING` inside Phase 1, before the charge is even attempted, means every committed `Order` now has a `Payment` row by construction; Phase 2 (step 14 below) only ever records an already-existing attempt's outcome, never creates the row. See `payment-domain-design.md`'s own updated field list for `Payment.attemptedAt`, the field that makes a crashed attempt (`PENDING` + `attemptedAt` `NULL`) distinguishable from a normal offline order genuinely awaiting the customer's money (`PENDING` + `attemptedAt` set).
...
```
unitCost = CostPriceProvider::costFor($variationId, $currency) ?? Money::zero($currency)
profit = amount - (unitCost × quantity)
```
...
**Resolved as of `operational-sales-domain-design.md` §3.13 stage 4a** — `SaleLine.unitCost` (nullable, `null` = genuinely unknown) is now populated on every fresh checkout SALE line, via `CheckoutLinePricingResult::unitCost()` (the raw value `costRecorded()` used to discard after computing `profit`) carried through `App\Services\SaleLineSnapshotBuilder` into `SaleLine::create()`. `profit` is now computed on NET, not on the pre-promotion `amount` this section originally described: `profit = netPaidAmount − (unitCost × quantity)`, and `netPaidAmount` itself when `unitCost` is `null`, never `netPaidAmount − 0` computed as if a zero cost had been verified. See §3.13's own "Profit, computed on net" subsection for the full rule.
