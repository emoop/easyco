...
## 12. One remaining flagged limitation, one now fixed
...
- **An unpriced Variation being ADDED to a cart is now a clean 422 — FIXED**, in a narrow, dedicated follow-up task. `EloquentPriceResolver::resolve()` had two distinct `RuntimeException` throw sites doing two genuinely different jobs — the "Regular Prices" system list itself missing (real system misconfiguration) versus a specific `priceableId` simply having no price yet (a normal, expected business state). Only the second now throws a dedicated, catchable `EasyCo\Pricing\Exceptions\PriceNotConfiguredException` (`extends RuntimeException`, so nothing that already caught the generic type breaks); the first throw site was deliberately left untouched — see `pricing-persistence-domain-design.md` §4.6. `CartController::store()` catches `PriceNotConfiguredException` specifically (never a broad `RuntimeException`, which would have also swallowed the genuinely-serious throw site) and returns a `422` naming the variation — confirmed by a real Feature test (adding an unpriced variation now returns `422`, not `500`).
  **A narrower residual gap, honestly not closed:** this fix only covers the add-to-cart path. `CartController::serializeCart()` — used by `GET /api/cart` and by every write response — still calls `resolve()` uncaught. A cart that already holds a line whose price was later unset entirely (an edge case, but a real one) would still surface an uncaught 500 on a subsequent read. Left this way because it was outside this follow-up's stated scope, not missed — worth closing in the same pass whenever this area is touched again.
