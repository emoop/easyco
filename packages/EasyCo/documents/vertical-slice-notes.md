...
**`Product::reconstituteFromStorage()` / `Variation::reconstituteFromStorage()`** — added directly to the domain classes as explicit, docblocked, persistence-layer-only factories, replacing an earlier `ReflectionProperty`-based hydration. Both trust that the data they're given already passed validation once, at write time, and do not re-run it. `Variation::reconstituteFromStorage()` is the one exception worth calling out precisely: it *does* still recompute the signature from the given assignments and run the existing consistency check — that's a cheap corruption detector, not business validation, and stays for every Variation regardless of how it's built. **Update, corrective pass:** at the time this slice first shipped, neither factory re-validated a Variation's combination against its Product's declared `VariationAxis` set, because axis declarations (`catalog_product_attributes` / `catalog_product_axis_values`) were never loaded from storage at all — a deliberate scope boundary, not a discovered gap, and `catalog-domain-design.md` §6 listed it as deferred work at the time. That gap is now closed: `EloquentProductRepository` rehydrates a VARIABLE product's real declared axes and threads them through `Product::reconstituteFromStorage()`'s new `$variationAxes` parameter, which calls the real `declareVariationAxes()` **before** variations are attached — see `catalog-domain-design.md` §3.10 for the mechanism and the new `UnsafeAxisRedeclarationException` invariant it introduced along the way (axes can no longer be redeclared once any STANDARD variation exists). Full suite after that corrective pass: **233 tests across all four packages** (Extensibility, Pricing, Catalog, App), zero failures.

**Update, v1.5:** the sentence directly above — "axes can no longer be redeclared once any STANDARD variation exists" — is superseded by `catalog-domain-design.md` §3.17. That blanket rule made two organic merchant operations impossible (enabling a new value on an existing axis; explicitly restoring an archived variation) and was replaced by a directional compatibility check: an axis change is refused only while it would actually orphan a **live (non-archived)** STANDARD variation's combination, never because of an archived one. This note is left here for historical accuracy about what the corrective pass described above actually shipped at the time; it does not describe current behavior.
...
| ~~SKU auto-generation~~ / barcode auto-generation | **SKU resolved as of the `catalog.product.base_sku`/`catalog.variation.sku` Hook work** (see §6) — both `Product::baseSku()` and `Variation::sku()` now auto-generate for real, with DB-constraint-driven collision retry. `barcode` still has no generation logic at all; it remains caller-supplied or absent, and `catalog_variations_barcode_unique` is the only thing preventing a collision, enforced at insert time, after the fact — unchanged from before this work. |
...
- Also not covered: `EloquentVariationRepository`'s query methods (`findById`/`findBySku`/`findByBarcode`/`findByProductId`) — verified manually via Tinker in this session, not by an automated test — real `PriceList`-backed resolution, and anything about load or performance under real traffic. (Axis-declaration validation on a reconstituted Product, listed here as not-covered when this slice first shipped, **is** covered now — see §2's update above and `catalog-domain-design.md` §3.10.)
...
## 6. Extensibility hooks — now proven, real SKU generation now done too

`packages/EasyCo/Extensibility` (a new, separate foundational package — see `extensibility-design-and-hooks.md`) now exists and is wired into the app: `HookRegistry` (a pure PHP action/filter registry, zero Laravel dependency) plus the Laravel-facing `Hook` facade, auto-discovered via `HookServiceProvider`. It's proven end-to-end through this same vertical slice, not just in isolation: `ProductController::store()` applies the `catalog.product.base_sku` and `catalog.product.slug` filters before calling `Product::createSimple()`.

**Update:** the `App\Providers\DemoHooksServiceProvider` proof-of-concept mentioned in earlier versions of this note (a demo listener replacing a literal input `"1"` with an in-memory counter) has been **deleted** — it served its purpose of proving the hook mechanism fires end-to-end through a real HTTP request, and that mechanism now has two real generators standing in its place: `App\Providers\CatalogSkuGeneratorServiceProvider`, backing both `catalog.product.base_sku` (a persistent, concurrency-safe sequence table) and the new `catalog.variation.sku` filter (`{baseSku}-{n}`, see `catalog-domain-design.md` §3.2 for why not attribute-value-based). `VariationCombinationGenerator::generate()`'s `$skuForCombination` callable — previously required, the only available injection point before this mechanism existed — is now optional; when omitted it throws rather than calling `Hook::apply()` itself, since Catalog is a domain package and must not (§2 of the hooks doc). `tests/Feature/CatalogSkuGeneratorTest.php` and `EloquentProductRepositoryVariationSkuCollisionTest.php` cover this end-to-end, replacing `ProductBaseSkuHookTest.php` (also deleted).
