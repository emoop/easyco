...
### 4.4.1 Set-based item lookup for batched resolution (`findByPriceListIdAndTargets()`)

`PriceListItemRepository::findByPriceListIdAndTarget()` (singular) answers "does THIS one target have an item here" — the right shape for a single admin form field. `findByPriceListIdAndTargets()` (plural, added alongside `PriceRange`) is its set-based sibling: one `whereIn` query against the same `(price_list_id, target_type, target_id, min_quantity)` index, for "which of THESE N targets have an item here."

This closes a real, previously-latent performance trap: `FixedItemsPriceLookup::forTarget()` used to call `findByPriceListId()` (the WHOLE list) and filter in PHP — harmless for one target at a time, but a system list like "Regular Prices" accumulates one row per priced Variation/Product store-wide, so a per-target `resolve()` loop over a product listing page would have loaded that entire list once per target — real, avoidable N+1-at-scale waste, not merely a style preference. `FixedItemsPriceLookup::forTargets()` is now the one real implementation of §4.3's fallback + §4.4's tier lookup; `forTarget()` is a thin wrapper delegating to it with a one-entry map, so both stay byte-for-byte identical in selection behaviour — verified by an equivalence test across the full matrix (variation-level, product-level fallback, quantity tiers, unpriced targets), not merely asserted.
...
**Exception behavior — two distinct failure modes, deliberately two distinct exception types.** `resolve()` can fail for two reasons that are not the same kind of problem, and are no longer conflated into one untyped `RuntimeException`:
- The reserved "Regular Prices" system list itself doesn't exist at all (§4.5 — never seeded). This means the pricing system isn't set up, a genuine system-misconfiguration state — `resolve()` still throws a plain, untyped `RuntimeException` for this, deliberately: nothing should ever catch it and turn it into a clean, swallowed error, because there is no sensible per-request recovery from "pricing isn't configured at all."
- The "Regular Prices" list exists, but carries no price for this specific `priceableId` — a normal, expected business state (a product exists and hasn't been priced yet). This throws `EasyCo\Pricing\Exceptions\PriceNotConfiguredException` (`extends RuntimeException`, so any existing broad `catch (RuntimeException)` still works unchanged) — added specifically so an external caller can catch *this* case without also silently swallowing the first, much more serious one. `EasyCo\Cart`'s `CartController::store()` is the first real caller: it catches `PriceNotConfiguredException` specifically around its add-to-cart price resolution and returns a clean `422` naming the variation — see `cart-domain-design.md` §12. A future Checkout flow will very likely need the same distinction.

**Historical correction (fixed in commit `11744f3`):** step 1's scope matching above describes BRAND/CATEGORY/TAG/ATTRIBUTE_VALUE scopes as working, and `EloquentPriceResolver` did implement that matching correctly from the start. But until `11744f3`, no caller ever actually populated `PriceContext::$matchingScopeReferenceIds` — so in practice, a `PriceList` scoped to a brand, category, tag, or attribute value silently never matched anything when resolved through Cart. A merchant could configure "Guess −20%," add a Guess product to a cart, and see no price change at all, with no error anywhere pointing at the cause. The fix was entirely app-layer, not in this package: `App\Services\CatalogScopeResolver` (in the root app, not `packages/EasyCo/Pricing`) assembles the real Catalog data — brand id, category ids, tag ids, attribute value ids — and `CartController` now passes it into every `PriceContext` it builds, keeping this package itself exactly as Catalog-agnostic as §1 requires. Worth checking for the same gap whenever a new caller starts building a `PriceContext`: a future Checkout flow must do the same assembly, or it will silently reintroduce the identical bug.
