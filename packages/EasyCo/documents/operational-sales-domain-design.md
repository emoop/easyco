...
### 3.13 Full sale-line snapshot for returns — stage 4a implemented (checkout writes the full snapshot)

**Status: stages 1 (this design), 2 (schema + `SaleLine` domain + reconstitution fix), 3 (per-line allocation in `PromotionDiscountCalculator`), and 4a (`CheckoutOrchestrator`/`InstallmentPlan` write the full snapshot via `SaleLineSnapshotBuilder`) are done.** See §6 for what's left (stage 4b — `SaleLine`'s constructor made private; stage 5 — the admin Order View). `OrderAdminReader` is still untouched (its own `finalUnitPrice`-aware read is stage 5's job, not this one) — every OTHER consumer named in stage 2's status line now populates/reads these fields on a real checkout.

**Stage 2's own temporary, tracked state:** `SaleLine`'s public constructor still enforces `productName`/`sku` for a fresh SALE line exactly as before this stage — UNCHANGED behaviour, deliberately — because `CheckoutOrchestrator` and `InstallmentPlan::buildSettlementSaleLines()` both still call `new SaleLine(...)` directly, not the new, stricter `SaleLine::create()`. The seven new fields stay optional, nullable constructor arguments for exactly that reason. `SaleLine::create()` (SALE-specific, enforces every §3.13 invariant) exists now but has no real caller yet — that's stage 4. The constructor's own docblock carries this same note and should be consulted (and the note removed) once stage 4 migrates both call sites to `create()`.
