...
### 3.13 Full sale-line snapshot for returns — stage 4 complete

**Status: stages 1 (this design), 2 (schema + `SaleLine` domain + reconstitution fix), 3 (per-line allocation in `PromotionDiscountCalculator`), 4a (`CheckoutOrchestrator`/`InstallmentPlan` write the full snapshot via `SaleLineSnapshotBuilder`), and 4b (`SaleLine`'s constructor made private) are all done.** See §6 for what's left (stage 5 — the admin Order View). `OrderAdminReader` is still untouched (its own `finalUnitPrice`-aware read is stage 5's job, not this one) — every OTHER consumer named in stage 2's status line now populates/reads these fields on a real checkout.

**Stage 4b, exactly per the constructor's own former "STAGE 4'S EXACT CHANGE" note:** `SaleLine`'s constructor is now `private`. Every caller goes through one of three named factories: `SaleLine::create()` (fresh SALE lines — moved the `productName`/`sku` Tier B check out of the constructor and into itself, so "required for a fresh SALE line" lives in exactly one place), `SaleLine::createNonSale()` (new — the fresh-construction path for every other type: RESERVATION/REFUND/SHIPPING/INSTALLMENT_PAYMENT, Tier A only, refuses `SaleLineType::SALE`), or `SaleLine::reconstituteFromStorage()` (persistence-layer only, Tier A only). The `$reconstituting` flag §3.12's amendment introduced as stage 2's temporary suppression mechanism is removed entirely — with Tier B living exclusively in `create()`, the constructor never needs to suppress anything, so `reconstituteFromStorage()` calls it directly with no gate at all. Every remaining `new SaleLine(...)` call site (production code and ~11 test files) was migrated to one of the three factories; no behaviour change for any caller except where a test's own fixture was itself formula-inconsistent with `create()`'s new invariants (one case, `EloquentTransactionRepositoryTest`'s multi-unit round-trip fixture — reported and fixed at review, not silently).

**`createNonSale()` will eventually be superseded, piecemeal, not all at once:** RESERVATION and REFUND will each get their own strict factory (mirroring `create()`'s own formula-invariant enforcement) once §3.13's own deferred REFUND rewrite (§3.4) and reservation-recording are actually designed and built. Until then, `createNonSale()` is the sole path for every non-SALE type — see its own docblock for why it should not be special-cased piecemeal in the meantime.
...
**§3.13's own implementation stages** (full sale-line snapshot for returns):

5. ~~Schema (migration for §3.13's new `SaleLine` columns) + the `SaleLine` domain changes themselves (`SaleLine::create()`, the reconstitution split, §3.12's amendment) + the retroactive `reconstituteFromStorage()` fix for `productName`/`sku`.~~ **Done** — `2026_09_24_000001_add_snapshot_fields_to_operational_sales_sale_lines_table.php` (real MySQL, `SHOW CREATE TABLE`/rollback-and-re-migrate confirmed); `SaleLine::create()`, the Tier A/Tier B validation split, and the shared `SaleLineMapper` (replacing both repositories' own duplicated `toDomainSaleLine()`) all shipped. At the time this stage shipped, `CheckoutOrchestrator`/`InstallmentPlan::buildSettlementSaleLines()` still called `new SaleLine(...)` directly, and the constructor stayed public with the seven new fields left optional for exactly that reason — both migrated in stages 7/8 below, not this one.
6. ~~Per-line allocation in `PromotionDiscountCalculator` (§3.13's Promotion allocation rule).~~ **Done** — `EasyCo\Pricing\Money::allocate(array $weights): array` (the largest-remainder method; the [5,5,1]/total-1 counter-example, ties-by-array-order, and every rejection case are all unit-tested in the Pricing package's own suite, including a seeded 1000-case randomized property test). `PromotionDiscountCalculator::eligibleAmountsPerLine()` now keeps the per-line eligible amounts `eligibleBase()`/`baseCappedByUsageLimit()` used to discard after summing, and allocates the already-computed total discount across them via `allocate()` — `amount()` is unchanged, byte-identical before/after (every pre-existing calculator test passes with zero edits). `PromotionDiscountResult::perLineShares()` exposes the breakdown, positionally aligned with `$applicableLines` (never keyed by `variationId` — see this section's own "Per-line keys" note above for why). No caller reads it yet — `CheckoutOrchestrator`/`CartController` still use only `amount()`/`discountCapped()`/`nominalAmount()`, unchanged; `perLineShares()` gets its first real consumer in stage 7 below.
7. ~~`CheckoutOrchestrator` writes the full snapshot via the new one-service-two-channels builder (§3.13 E-D4), migrating both `new SaleLine(...)` call sites to `SaleLine::create()`, and `profit` moves to being computed on net (§3.13).~~ **Stage 4a done** — `App\Services\SaleLineSnapshotBuilder` (Web Checkout's caller today; a future POS flow is its second, unchanged) builds every checkout SALE line via `SaleLine::create()`; `CheckoutOrchestrator::resolvePromotion()` now returns a per-line `promotionDiscountShare` for EVERY priced line (zero for a non-applicable one, never omitted); `InstallmentPlan::buildSettlementSaleLines()` also moved to `SaleLine::create()` (§3.13's own D7) — a reserved line settled without its own §3.13 snapshot now throws instead of silently carrying nulls through (a real, deliberate behaviour change from stage 2's interim posture, not an oversight — see `InstallmentPlanTest`'s own replaced test for the before/after). `recordPayment()` was reordered to build settlement lines BEFORE mutating any plan state, so that throw leaves a plan completely untouched. D6's order-level reconciliation (`Σ promotionDiscountShare == order discount`, `Σ netPaidAmount == order total`) is checked by `CheckoutOrchestrator` itself before either write, via a new `SaleLineOrderReconciliationException` — proven to actually abort-with-nothing-written by a test that forces a mismatch through a corrupting `SaleLineSnapshotBuilder` binding, since the real pipeline cannot organically produce one. `soldAttributes` (D5) is read in one batched, 4-query call per checkout regardless of cart size (`SaleLineSnapshotBuilder::batchLoadSoldAttributes()`, query-count-tested 2 vs. 10 lines) — **and the axis-order question D5 asked has a real answer, not a guess: there is no authoritative variation-axis order anywhere in this codebase.** `catalog_product_attributes.sort_order` exists as a column but is always written as the literal `0` (`EloquentProductRepository::persistVariationAxes()`, both call sites) and `loadVariationAxes()` doesn't even `ORDER BY` it — confirmed by reading the real installed source, not assumed. `soldAttributes` is therefore sorted by `attribute_definition_id` ascending, deterministic and stable, not left to MySQL's unordered row-return order; see `SaleLineSnapshotBuilder`'s own class docblock for the full finding.
8. ~~`SaleLine`'s constructor made private, the productName/sku Tier B check moved into `create()`, the `$reconstituting` flag removed.~~ **Stage 4b done** — exactly per the constructor's former "STAGE 4'S EXACT CHANGE" note. New `SaleLine::createNonSale()` is the fresh-construction path for every type except SALE (Tier A only, refuses SALE); `reconstituteFromStorage()` now calls the private constructor directly, no suppression flag needed. Every remaining `new SaleLine(...)` call site — production code was already migrated in stage 4a; this stage covered the ~11 test files — was migrated to `create()`/`createNonSale()`/`reconstituteFromStorage()` as appropriate. One test fixture (`EloquentTransactionRepositoryTest`'s multi-unit round-trip: amount 2500, quantity 3) turned out to be formula-inconsistent with `create()`'s new `amount == finalUnitPrice × quantity` invariant (2500 isn't evenly divisible by 3) — its input values were adjusted (2500→3000) to the nearest formula-consistent equivalent preserving the same test intent, reported at review rather than silently changed.
9. Admin Order View (`admin-panel-design.md` §14) surfaces the new fields — not started.
