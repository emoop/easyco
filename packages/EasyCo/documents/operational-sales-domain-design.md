...
### 3.13 Full sale-line snapshot for returns — stage 2 implemented (schema + SaleLine domain + reconstitution fix)

**Status: stages 1 (this design) and 2 (schema, `SaleLine` domain, reconstitution fix) are done.** See §6 for the remaining implementation stages (allocation in the calculator, Checkout writing the full snapshot, the admin Order View). Stage 2 delivered exactly the fields/invariants below on `SaleLine` itself and its persistence — `CheckoutOrchestrator`, `PromotionDiscountCalculator`, `OrderAdminReader`, and any admin UI are UNTOUCHED; nothing populates these fields on a real checkout yet.
...
7. `CheckoutOrchestrator` writes the full snapshot via the new one-service-two-channels builder (§3.13 E-D4), migrating both `new SaleLine(...)` call sites to `SaleLine::create()`, and `profit` moves to being computed on net (§3.13) — not started. **This stage also makes `SaleLine`'s constructor private, moves the productName/sku Tier B check out of it and into `create()` (so "required for a fresh SALE line" lives in exactly one place), and removes the `$reconstituting` flag entirely** — see `SaleLine.php`'s own constructor docblock ("STAGE 4'S EXACT CHANGE") for the precise mechanics, recorded now so stage 7 doesn't have to rediscover them.
